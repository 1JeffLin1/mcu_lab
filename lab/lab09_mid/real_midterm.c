...
void main(void)
{
    ...
    unsigned char mem[4], mem_up[4];
    unsigned char i_chg, setting, setmode, flash_cnt;
    ...
    _pec = 0;              //  ] w PE  ? X; PE[3:0]=com port PE[7:4]:keyboard col
    ...
    _c1os = 1;           // pa5 is disconnected to c1x and purly acts as an I/Opin
                         //
    _acerl = 0b00000111; // PA0, PA1, PA2: AN  J; 0: ] w PA  ?@  I/O A DAN     J
    ...
    for (i_chg = 0; i_chg < 4; i_chg++)
    {
        mem_up[i_chg] = EEPROM_RD(i_chg + 4);
    }
    uplimit = 1000 * mem_up[3] + 100 * mem_up[2] + 10 * mem_up[1] + mem_up[0];
    ...
    while (1)
    {
        ...
        else if (setmode == 2)
        {
            ...
            for (i_chg = 0; i_chg < 4; i_chg++)
            {
                EEPROM_WR(i_chg + 4, dig_bcd[i_chg]);
            }
        }
