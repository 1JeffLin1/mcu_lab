...
void main(void) // 主函式
{
	...
	_t2cp0 = 0;			 // pc3/TP2_0 is disconnected to TM2
	_t2cp1 = 0;			 // pc4/TP2_1 is disconnected to TM2
						 //	_t1bcp2=0; // pc5/TP0_1/TP1B_2 is disconnected to TM1 (default=0)
						 //	_t0cp1=0;  // pc5/TP0_1/TP1B_2 is disconnected to TM0
						 //
	...
	/* ## 若有需要可以從EEPROM取出uplimit的四個位數的值 ##
		for(i_chg=0; i_chg<4; i_chg++)
		{
			pei[i_chg]=EEPROM_RD(i_chg+4);
		}
		uplimit=1000*pei[3]+100*pei[2]+10*pei[1]+pei[0];
	*/
	uplimit = 3 * lowlimit;
	...
	while (1)
	{
		...
		/*      // ## 若有需要可以，轉換VR1的ADC值
					...
					adc1h=_adrh;
					adc1l=_adrl;
					uplimit=adc1h*256+adc1l;
					bin2bcd(uplimit);
					for(i_chg=0; i_chg<4; i_chg++)
					{
						EEPROM_WR(i_chg+4, dig_bcd[i_chg]); //save as address 4~7
					}
				}
		*/
		...
		if (setmode != 0)
		{
			if ((adc0h * 256 + adc0l < 400) || (adc0h * 256 + adc0l > 1350))
			{
				if (_t0on == 0)
				{
					_tm0al = 0x08;
		...
		else
		{
			if (adc2h * 256 + adc2l > uplimit)
			{
				...
				_pc0 = 0;
				_pc1 = 1;
				_pc2 = 1; // 青色，pc0,pc1,pc3=紅,藍,綠
			}
			else if (adc2h * 256 + adc2l < lowlimit)
			{
				_t0on = 0;
				if (_t0on == 0)
				{
		...
		for (dig_pos = 0; dig_pos < 4; dig_pos++)
		{
			...
			Led7_seg = led7seg[dig_bcd[dig_pos]]; // change the LED pattern
			Led7_com = led7com[dig_pos];		  // change the seleted digit position
			delay((1 - dig_pos / 3) * 50);		  // longer delay --> higher LED brightness
		}
		//
	}
}
