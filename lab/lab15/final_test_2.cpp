...
/////////////////////////////////////////////////
const unsigned int judge;      // 判斷是否需要超過2秒
const unsigned char times = 0; // 超過2秒用此記數
////////////////////////////////////////////////
...
void main(void) // 主函式
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (adc1_val <= 4095 && adc1_val >= 3072) // 2秒
            {
                judge = 0;
                _tm2al = (domain[3] + 4096) % 256;
                _tm2ah = (domain[3] + 4096) / 256;
            }
            else if (adc1_val <= 3071 && adc1_val >= 2048) // 1秒
            {
                judge = 0;
                _tm2al = (domain[2] + 4096) % 256;
                _tm2ah = (domain[2] + 4096) / 256;
            }
            else if (adc1_val <= 2047 && adc1_val >= 1024) // 0.5秒
            {
                judge = 0;
                _tm2al = (domain[1] + 4096) % 256;
                _tm2ah = (domain[1] + 4096) / 256;
            }
            else if (adc1_val <= 1023 && adc1_val >= 0) // 0.25秒
            {
                judge = 0;
            ...
            /*	// 6段
                        else if (adc1_val <=  && adc1_val >= )   // 假設為4秒
                        {
                            judge=4;
                            _tm2al = (domain[2] + 4096) % 256;  // 用一秒的來記數
                            _tm2ah = (domain[2] + 4096) / 256;
                        }
                        else if (adc1_val <=  && adc1_val >= )  // 假設為8秒
                        {
                            judge=8;
                            _tm2al = (domain[2] + 4096) % 256;  // 用一秒的來記數
                            _tm2ah = (domain[2] + 4096) / 256;
            ...
            */
            /*   //8段
                        else if (adc1_val <=  && adc1_val >= )  ////////////////////////// 可試試奇數秒
                        {
                            judge=0;
                            _tm2al = () % 256;
                            _tm2ah = () / 256;
                        }
                        else if (adc1_val <=  && adc1_val >= )
                        {
                            judge=0;
                            _tm2al = () % 256;
                            _tm2ah = () / 256;
