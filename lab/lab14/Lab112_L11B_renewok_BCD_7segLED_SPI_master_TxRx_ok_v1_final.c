...
// The SPI modual MPU is chosen as a master device which transmits the ADC_value/32 and receives
//    the ADC_value/32 from a slave device. 
...
//   (3) SPI master --> SPI slave : pb5 --> pb5, pa7 --> pa7, pa6 --> pa5, pa5 --> pa6
...
unsigned char adc0h, adc0l, channel, now_ch; //, adc1h, adc1l; // , adc2h, adc2l;
//  
//
...
//	SPI: _pa5=SDO, _pa6=SDI, _pa7=SCK, _pb5=scs
	_pac |= 0b01000000 ; 	// 0 : 為輸出; 1 : 為輸入
	_papu |= 0b01000000; // pull up PA6 
//	_pbc6=0;
	_pbc5=0; 	// 0 : 為輸出; pb5:slave scs; pb6:master scs
...
//             011: SPI master mode; SPI clock is fTBC 
	_simc0=0b00100000;	//[7:5]=101:SPI slave, 001: fSYS/16 ;[1]=1:SIMEN
	SCS0 = 1;	
...
// scanning DI0splay of 4-DI0git seven-segment LEDs	
		for( disp_num = 0 ; disp_num < 100 ; disp_num++ ) 	
		{
